...
export async function createUser(req, res) {
 ...
 if ( !req.body.firstname || !req.body.email || !req.body.lastname || !req.body.password) {
    ...
    const userObj = new User( req.body.firstname, req.body.lastname, req.body.email, req.body.password)
...
export async function updateUser(req, res) {
    ...
    const { id } = req.params;
    if (!id || !firstname || !email || !lastname || !password) {
        return res.status(400).json({ message: "missing data" })
    }
    ...
    const userObj = new User(firstname, lastname, email, password, user.admin, user.history, user.vehicles, user.card ,user.chat)
...
export async function AddCard(req, res) {
    ...
    await User.AddCardToUser(credit_card, id);
...
export async function AddHistory(req, res) {
    const { id } = req.params
    const { stationName, address, city, operator, connector, power, speed } = req.body
    let user = await User.AllUsers(id);
    if (!user) {
        return res.status(404).json({ message: 'User Not Found' })
    }
    if (!stationName || !address || !city || !operator || !connector || !power || !speed) {
        return res.status(400).json({ message: "missing data" })
    }
    const station={stationName:stationName,address:address,city:city,operator:operator,connector:connector,power:power,speed:speed}
